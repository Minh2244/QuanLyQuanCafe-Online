...
<script>
    $(document).ready(function() {
        ...
        $('.quantity-btn.minus').click(function(e) {
            e.preventDefault();
            const id = $(this).data('id');
            const currentQty = parseInt($('#qty-' + id).text());
            if (currentQty > 1) {
                updateQuantity(id, -1);
            }
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        // Cập nhật thành tiền
                        $('#thanh-tien-' + id).text(response.item_total + ' VNĐ');

                        // Cập nhật tổng tiền
                        $('tfoot strong:last').text(response.cart_total + ' VNĐ');

                        // Đã xóa phần cập nhật số lượng trong giỏ hàng trên menu
                    }
                },
                error: function() {
